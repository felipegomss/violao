      ...
      <div className="flex items-center justify-between gap-6 border-b border-ink/12 px-8 py-5">
        <div className="flex items-center gap-2">
          <Link
            href={backHref}
            aria-label="Voltar"
            className={`-ml-3 flex h-11 w-11 items-center justify-center rounded-lg text-faint transition-colors duration-150 hover:text-ink ${FOCUS}`}
          >
            <ChevronLeft size={18} strokeWidth={2} />
          </Link>
          <h2 className="font-editorial font-semibold text-[32px] leading-none">{title}</h2>
        </div>
      </div>
      ...
      <div className="grid min-h-0 flex-1 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="flex min-h-0 flex-col bg-[#efe7d5]">
            ...
            {PANEL.map((d) => {
              ...
              return (
                <div key={d.key} className="border-b border-ink/9 py-3">
                  <label
                    htmlFor={`f-${d.key}`}
                    className="mb-1.5 block font-cifra text-[11px] lowercase text-faint"
                  >
                    {d.label}
                  ...
                  <input
                    ...
                    className={`h-11 w-full rounded-lg border-[1.5px] bg-[#fbf7ee] px-2.5 font-cifra text-[14px] text-ink outline-none transition-colors duration-150 ${FOCUS} ${
          ...
          <div className="border-t border-ink/12 px-5 py-4">
            {hasMissing && (
              <div className="mb-2.5 font-cifra text-[11px] leading-relaxed text-rust">
                falta preencher: {missing.map((m) => m.label.toLowerCase()).join(', ')}
              </div>
            )}
            {state?.errors && (
              <div className="mb-2.5 font-cifra text-[11px] text-rust">
                {Object.values(state.errors).flat()[0]}
              </div>
            )}
            <Btn type="submit" disabled={pending || hasMissing} className="w-full">
