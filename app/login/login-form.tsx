...
import { requestMagicLink, type MagicState } from '@/app/actions/auth'
...
export function LoginForm({ linkError }: { linkError?: boolean }) {
  ...
  const [magic, magicAction, magicPending] = useActionState<MagicState, FormData>(
    requestMagicLink,
    undefined,
  )
  ...
  if (magic?.sent) {
    return (
      <div className="w-full max-w-[340px]">
        ...
        <h3 className="font-editorial text-[32px] font-semibold leading-tight text-ink">
          Link enviado
        </h3>
        <p className="mt-3 font-editorial text-[19px] italic text-soft">
          Mandamos um link pra <span className="not-italic text-ink">{email}</span>. Confere a caixa
          (e o spam). Ele vale por 15 minutos.
        </p>
        <Btn
          type="button"
          variant="ghost"
  ...
  return (
    <div className="w-full max-w-[340px]">
      ...
      <form action={magicAction} className="mt-8">
        <label htmlFor="email" className="mb-2 block font-cifra text-[11px] lowercase text-faint">
          email
        </label>
        <input
          id="email"
